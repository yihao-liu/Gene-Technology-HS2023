...
def evaluate_few_shot(splitter: Union[StratifiedShuffleSplit], folder_paths: List[str], device: torch.device) -> List[float]:
    scores = []
    ...
    for folder_path in tqdm(folder_paths, total=len(folder_paths)):
        ...
        for train_indices, val_indices in tqdm(splitter.split(X=df, y=df["output"], groups=df["output"]), total=splitter.n_splits, leave=False):
            ...
            scores.append(top_10_accuracy(output, true_labs))
...
if __name__ == "__main__":
    ...
    print("Evaluating for train_size=0.1...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.1,
            random_state=42,
    ...
    print("Mean: %f, Std: %f" % (float(np.mean(scores)), float(np.std(scores))))
    
    print("Evaluating for train_size=0.2...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.2,
    ...
    print("Evaluating for train_size=0.3...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.3,
    ...
    print("Evaluating for train_size=0.4...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.4,
    ...
    print("Evaluating for train_size=0.5...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.5,
    ...
    print("Evaluating for train_size=0.6...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.6,
    ...
    print("Evaluating for train_size=0.7...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.7,
    ...
    print("Evaluating for train_size=0.8...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.8,
    ...
    print("Evaluating for train_size=0.9...")
    scores = evaluate_few_shot(
        FewShotSplit(
            n_splits=1000,
            train_size=0.9,
