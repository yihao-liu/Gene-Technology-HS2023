...
def read_dataset():
    ...
    train_values_grouped_path = (
        os.path.join(DATASET_DIR, "train_values_grouped.csv")
        if not data_preparation_config.extra_data
        else os.path.join(DATASET_DIR, "extra_train_values_grouped.csv")
    )
    ...
    if not data_preparation_config.unknown_engineered:
        c.df = c.df[c.df["I7FXTVDP"] == 0.0].reset_index(drop=True)
        c.output_columns = c.output_columns.drop("I7FXTVDP")
